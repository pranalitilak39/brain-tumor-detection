...
def get_grad_cam(model, img_array, layer_name="last_conv_layer"):
    ...
    # Build Grad-CAM model safely for Sequential model
...
if uploaded_file is not None:
    ...
    # prediction
    pred = model.predict(img_array)[0][0]  # single probability
    if pred > 0.5:
    ...
    tumor_prob = float(pred)
    st.write({"No Tumor": 1 - tumor_prob, "Tumor": tumor_prob})
